...
I18n = {
  ...
  findString: function findString(msgPath) {
    var matchingString, path, err;
    // no path? no string
    if (_.isEmpty(msgPath) || !_.isString(msgPath)) {
      err = new Error('i18n:t() - received an empty path.');
      logger.error(err);
      return '';
    }
    ...
    if (_.isNull(matchingString)) {
      err = new Error('Unable to find matching path [' + msgPath + '] in locale file.\n');
      logger.error(err);
      matchingString = '';
    }
